...
export class SelectComponent implements OnInit {
  ...
  addPassenger(i: number){
    if (i > 0 && this.passengerCount === environment.MAX_PASSENGER_COUNT){
      this.openAlert({title: 'Not Allowed', text: ['You reached maximim passenger count.'], type: null});
      return;
    }
    if (i < 0 && this.passengerCount === environment.MIN_PASSENGER_COUNT){
      this.openAlert({title: 'Not Allowed', text: ['Passenger count must be bigger then 0.'], type: null});
      return;
    }
  ...
  addVehicle(i: number){
    if (i > 0 && this.vehicleCount === environment.MAX_VEHICLE_COUNT){
      this.openAlert({title: 'Not Allowed', text: ['You reached maximim vehicle count.'], type: null});
      return;
    } else if (i < 0 && this.vehicleCount === environment.MIN_VEHICLE_COUNT) {
      return;
    } else if (i>0 && this.vehicleCount === this.passengerCount){
      this.openAlert({title: 'Not Allowed', text: ['Not enough passenger.', 'Please first add passenger.'], type: null});
      return;
    };
